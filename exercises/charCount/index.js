...
const charCount = str => {
    ...
    for (let i = 0; i < str.length; i++) {
        if (result[str[i]] > 0 ) { // means its already in there. since its in there, increment the count of that char
            result[str[i]]++;
            console.log(`incrementing "${str[i]}" because it is already in the object`)
        } else {
